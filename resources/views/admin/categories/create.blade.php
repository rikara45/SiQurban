...
<head>
    ...
    <title>Tambah Kategori - {{ config('app.name', 'Laravel') }}</title>
                        ...
                        <div class="p-6 text-white">
                            <form action="{{ route('admin.categories.store') }}" method="POST">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="name" class="block mt-1 w-full input-glass" type="text" name="name" :value="old('name')" required autofocus />
                                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                                </div>
                                <div class="mt-4">
                                    <x-input-label for="description" :value="__('Deskripsi (Opsional)')" class="text-gray-300"/>
                                    <textarea id="description" name="description" class="block mt-1 w-full rounded-md shadow-sm input-glass" rows="4">{{ old('description') }}</textarea>
