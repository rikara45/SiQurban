                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('penjual.animals.update', $animal) }}" method="POST">
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="price" class="block mt-1 w-full" type="number" name="price" :value="old('price', $animal->price)" required />
                            <x-input-error :messages="$errors->get('price')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="weight" :value="__('Berat (kg)')" />
                            <x-text-input id="weight" class="block mt-1 w-full" type="number" step="0.1" name="weight" :value="old('weight', $animal->weight)" required />
                        ...
                        <div class="mt-4">
                            ...
                            <select name="status" id="status" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                <option value="sold" {{ old('status', $animal->status) == 'sold' ? 'selected' : '' }}>Terjual</option>
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="description" name="description" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $animal->description) }}</textarea>
                             <x-input-error :messages="$errors->get('description')" class="mt-2" />
                        </div>

                        {{-- Note: Photo editing is more complex, so we skip it for now in the edit form --}}
                        <p class="text-sm text-gray-500 mt-4">Manajemen foto belum diimplementasikan di halaman edit.</p>
